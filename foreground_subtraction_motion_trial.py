...
def get_foreground_background_frames(play_video_window=False):
    ...
    # args = ["Stairs_490_270_346", 0.4, 15, 0, 8, [0, 0, 5, 5]]
    # args = ["test1_480_270_404", 0.95, 45, 0.4, 5, [5, 1, 9,12]]
    args = ["test2_480_270_631", 0.7, 35, 0.2, 3, [5, 1, 2, 3]]
    ...
    # imgs = readMP4(path_prefix, "done.MP4")
    ...
    for i in range(1, len(imgs)):
        ...
        u, c = np.unique(np.round(subsampled_mag, 1), return_counts=True)
        ...
        for x in range(edge_buffers[0], m-edge_buffers[1]):
            for y in range(edge_buffers[2], n-edge_buffers[3]):
                if min(abs(subsampled_ang[x][y] - most_common_ang),
                    ...
                    mask[x][y] = 1
                    diff_count += 1
        # compensate for moments when angular difference in frame is minimum
        if i > 400 and diff_count < 11:
            for x in range(edge_buffers[0], m - edge_buffers[1]-2):
                for y in range(edge_buffers[2]+5, n - edge_buffers[3]):
                    if abs(subsampled_mag[x][y] - most_common_mag) > 0.8:
                        mask[x][y] = 1
                        diff_count += 1
    ...
    return foreground_frames, background_frames, object_shapes, final_frame_masks, imgs


if __name__ == '__main__':
    # play_video_window is set to True here to display video as rendering for debug purposes
    foreground_frames, background_frames, object_shapes, final_frame_masks, imgs = get_foreground_background_frames(True)